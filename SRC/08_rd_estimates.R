...
# Running statewise RDD with and without controls
state_list <- c(
  "karnataka",
  "kerala",
  "andhra pradesh",
...
for (i in state_list) {
  ...
  srdd[[i]] <- tryCatch({
    rdrobust(y = df$pm25_avg18, x = df$d2c, all = TRUE, covs = z_controls)
  }, error = function(e) {
  ...
  srdd2[[i]] <- tryCatch({
    rdrobust(y = df$pm25_avg18, x = df$d2c, all = TRUE)
  }, error = function(e) {
...
#Running aggregate RDD with and without controls
